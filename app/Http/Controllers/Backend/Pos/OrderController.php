...
class OrderController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $orders = Order::with('cottage')->get();
            return DataTables::of($orders)
                ->addIndexColumn()
                ->addColumn('saleId', fn($data) => "#" . $data->id)
                ->addColumn('customer', fn($data) => $data->cottage->name ?? '-')
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'customer_id.exists' => 'The selected cottage does not exist.',
        ...
        $order = Order::create([
            ...
            'order_taker_id' => $request->user()->id,
        ]);
        $totalAmountOrder = 0;
    ...
    {
        $order = Order::with(['cottage', 'products.product'])->findOrFail($id);
