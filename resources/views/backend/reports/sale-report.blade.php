                ...
                <table id="datatables" class="table table-hover">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{ $index + 1 }}</td>
                      <td>#{{$order->id}}</td>
                      <td>{{ trim($order->customer_name) !== '' ? $order->customer_name : ($order->cottage->name ?? '-') }}</td>
                      <td>{{ $order->created_at->format('d-m-Y') }}</td>
                      <td>
                        @if($order->products && count($order->products) > 0)
                            @foreach($order->products as $product)
                                {{$product->product->name ?? "Unknown"}} (x{{$product->quantity}})<br>
                            @endforeach
                        @else
                            {{$order->total_item}}
                        @endif
                      ...
                      <td>
                        {{ App\Models\User::find($order->user_id)->name ?? "User #" . $order->user_id }}
                      </td>
